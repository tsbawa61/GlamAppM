...
// Subservices toggle state (full view vs. checked-only view)
let _utilizeSubservicesShowCheckedOnly = false;

export function toggleUtilizeSubservicesView() {
    const container = document.getElementById("container-utilize-subservices");
    const icon = document.getElementById("icon-toggle-subservices");
    if (!container || !icon) return;

    _utilizeSubservicesShowCheckedOnly = !_utilizeSubservicesShowCheckedOnly;

    const formChecks = container.querySelectorAll('.form-check');
    formChecks.forEach(div => {
        const checkbox = div.querySelector('input[type="checkbox"]');
        if (_utilizeSubservicesShowCheckedOnly) {
            // Show only checked items (with staff allocation)
    ...
    // Switch icon
    if (_utilizeSubservicesShowCheckedOnly) {
        icon.innerHTML = '<path d="M1 1l22 22M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19"/><path d="M14.12 14.12a3 3 0 0 1-4.24-4.24"/>';
    } else {
...
export async function displayWalletLedger() {
    ...
    try {
        ...
        for (const pkg of packages) {
            ...
            html += `<div class="mb-2" style="border-bottom:1px solid #eee;padding-bottom:8px;">
                        <div style="display:flex; justify-content:space-between; align-items:center;">
                            <span>${pkg.packName} <span style="color:#999;">-</span></span>
                            <span>
                                <span style="color:green;">+₹${Number(pkg.totalAmount).toLocaleString("en-IN")}</span>
                                <span style="font-size:0.8em;color:#999;">( ${createdDate} )</span>
                            </span>
                        </div>
                     </div>`;
            ...
            if (totalCost > 0) {
                html += `<div class="mb-2" style="border-bottom:1px solid #eee;padding-bottom:8px;">
                            <div style="display:flex; justify-content:space-between; align-items:center;">
                                <span>${pkg.packName} <span style="color:#999;">-</span></span>
                                <span>
                                    <span style="color:red;">-₹${totalCost.toLocaleString("en-IN")}</span>
                                    <span style="font-size:0.8em;color:#999;">(Services Cost)</span>
                                </span>
                            </div>
                          </div>`;
        ...
        html += `<div style="border-top:2px solid #333;padding-top:8px;margin-top:8px;font-weight:bold;color:green;">
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                        <span>NET <span style="color:#999;">-</span></span>
                        <span>
                            <span style="color:green;">₹${Number(walletAmt).toLocaleString("en-IN")}</span>
                            <span style="font-size:0.8em;color:#999;">(Wallet Amt)</span>
                        </span>
                    </div>
                 </div>`;
...
export async function renderVisitHistory(allotId, customerNo, packData = null) {
    ...
    try {
        ...
        // Populate customer name in header (appears only once) — prefer the
        // selected customer dropdown text, fall back to the log's customerName
        const custNameEl = document.getElementById("utilize-visit-history-customer");
        if (custNameEl) {
            const custSel = document.getElementById("utilize-customer-select");
            const selName = custSel?.selectedOptions?.[0]?.textContent?.trim();
            custNameEl.textContent = selName || (logs.length > 0 ? (logs[0].customerName || "—") : "—");
        }
...
export function resetOldVisitUI() {
    ...
    // Reset subservices toggle to full view
    if (_utilizeSubservicesShowCheckedOnly) {
        const container = document.getElementById("container-utilize-subservices");
        if (container) {
            const formChecks = container.querySelectorAll('.form-check');
            formChecks.forEach(div => div.style.display = 'block');
        }
        _utilizeSubservicesShowCheckedOnly = false;
    }
    const icon = document.getElementById("icon-toggle-subservices");
...
export async function deleteOldVisitRecord() {
    ...
    try {
        ...
        // Update customer wallet: increase by calculatedValueCost
        const custQ = query(collection(db, COL.USERS),
            where("ownerUserNo", "==", activeSessionUser.ownerUserNo),
            where("userNo", "==", custNo));
        const custSnap = await getDocs(custQ);
        if (!custSnap.empty) {
            ...
            const addedValue = Number(logData.calculatedValueCost || 0);
            const newWallet = currentWallet + addedValue;
...
export async function processVisitDeductionFormSubmission(e) {
    ...
    if (isOldVisit) {
        ...
        try {
            ...
            // Initialize UI elements after modification
            resetOldVisitUI();
            document.getElementById("utilize-visit-new").checked = true;
            await renderVisitHistory(allotId, custNo, pData);
            ...
            // Clear services container and hide financial details
            const container = document.getElementById("container-utilize-subservices");
            if (container) container.innerHTML = "";
            const financialEl = document.getElementById("utilize-pack-financial");
            if (financialEl) financialEl.style.display = "none";
            ...
            // Hide old visit wrapper
            const oldVisitWrapper = document.getElementById("utilize-old-visit-wrapper");
            if (oldVisitWrapper) oldVisitWrapper.style.display = "none";
